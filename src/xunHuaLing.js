async function xunHuaLing(date) {
  const result = await fetch(
    `https://xiaoce.fun/api/v0/quiz/daily/xunhualing/getTemp?date=${date}`,
    {
      headers: {
        "accept": "*/*",
        ...
        "sec-gpc": "1",
        "Referer": "https://xiaoce.fun/xunhualing",
      },
    }
  );
  const json = await result.json();
  const answer = json.data.data.content;
  ...
  btns.forEach((b) => {
    if (b.textContent.includes("猜")) b.click();
  });
}
