async function caiYing(date) {
  const result = await fetch(`https://xiaoce.fun/api/v0/quiz/daily/lol/get?date=${date}`, {
    headers: {
      accept: "*/*",
  ...
  for (const ele of document.querySelectorAll(".ant-card-body")) {
    if (answer.replaceAll(" ", "").includes(ele.textContent.replaceAll(" ", ""))) {
      ele.click();
      break;
    }
  }
